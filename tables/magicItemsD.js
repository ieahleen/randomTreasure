...
export function magicItemsD() {
	...
	if (n <= 50) return `${randomItemFromArray(spells6)} spell scroll (6th level)`;
	if (n <= 57) return `${randomItemFromArray(spells7)} spell scroll (7th level)`;
	if (n <= 62)
	...
	if (n <= 92) return `${randomItemFromArray(spells8)} spell scroll (8th level)`;
